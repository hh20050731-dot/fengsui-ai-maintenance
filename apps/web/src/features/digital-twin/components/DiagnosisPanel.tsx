...
export function DiagnosisPanel({ scenario, deviceId, deviceName, selectedNode, partResolution }: { scenario: FaultScenario; deviceId: string; deviceName: string; selectedNode: ModelNodeInfo | null; partResolution: PartResolution | null }) {
  ...
  return <section className="twin-panel twin-diagnosis">
    ...
    <div className="twin-diagnosis__body">
      ...
      <div className="twin-node-card"><div className="flex items-center gap-1.5 font-medium text-[var(--twin-text)]"><MousePointer2 size={13} />已选模型节点</div><div className="twin-data mt-1.5 break-all">{selectedNode?.name ?? '未选择（点击模型部件查看）'}</div>{selectedNode && <div className="twin-muted mt-1">{selectedNode.type}{selectedNode.vertexCount ? ` · ${selectedNode.vertexCount.toLocaleString()} 顶点` : ''}</div>}</div>
