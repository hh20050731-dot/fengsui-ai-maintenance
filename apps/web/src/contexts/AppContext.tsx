...
export interface IntegrationStatus {
  ...
  safeErrorCode: 'FEISHU_AUTH_INVALID' | 'FEISHU_AUTH_UNAVAILABLE' | 'FEISHU_API_UNAVAILABLE' | null;
  partial?: boolean;
  sso: string; bitable: string; robot: string; aiProvider: string; version: string; lastSyncAt: string; missingConfig: string[];
  capabilities: Record<'equipment' | 'telemetry' | 'health' | 'alerts' | 'workOrders' | 'spareParts' | 'spareTransactions' | 'knowledge' | 'operationLogs', {
    mode: 'mock' | 'feishu';
    effectiveMode: 'mock' | 'feishu';
    configured: boolean;
    authenticated: boolean;
