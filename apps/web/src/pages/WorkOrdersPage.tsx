...
function WorkOrderDetail({ order, parts, onClose }: { order: WorkOrder; parts: SparePart[]; onClose: () => void }) {
  ...
  const resync = useMutation({ mutationFn: () => postJson<WorkOrder>(`/work-orders/${encodeURIComponent(getWorkOrderTransitionIdentifier(order))}/resync`, {}), onSuccess: (updated) => { queryClient.setQueryData(['work-order', getWorkOrderNo(order)], updated); void queryClient.invalidateQueries({ queryKey: ['work-orders'] }); setToast(updated.syncStatus === 'synced' ? '重新同步成功' : '已发起重新同步'); } });
  ...
  return <><Modal title="维修工单详情" onClose={onClose} width="max-w-6xl">
    <div className="work-order-detail__header"><div><div className="flex flex-wrap items-center gap-3"><h3 className="industrial-data">{getWorkOrderNo(order)}</h3><StatusBadge status={order.status} /><RiskBadge level={order.riskLevel} />{order.syncStatus && <span className="work-order-sync"><i className={order.syncStatus === 'synced' ? 'is-synced' : ''} />飞书同步：{order.syncStatus === 'synced' ? '已同步' : order.syncStatus === 'failed' ? '失败' : '同步中'}</span>}{order.notificationStatus && <span className="work-order-sync">卡片通知：{order.notificationStatus === 'sent' ? '已发送' : order.notificationStatus === 'failed' ? '发送失败' : '待发送'}</span>}</div><button onClick={() => navigate(`/equipment/${order.deviceId}`)}>{order.deviceName} · {order.deviceId}</button></div><div><span>负责人</span><strong>{order.assignee}</strong></div></div>
    {order.syncStatus === 'failed' && <div className="mt-4"><AlertBanner tone="danger" title="飞书同步失败" description={order.syncMessage ?? '本地工单已保留，可重新同步。'} /><button className="btn-secondary mt-2" disabled={resync.isPending} onClick={() => resync.mutate()}>{resync.isPending ? '正在同步…' : '重新同步飞书'}</button></div>}
    <IndustrialPanel title="工单状态轨道" meta="状态转换沿用现有工单状态机" className="mt-4"><WorkOrderRail status={order.status} /></IndustrialPanel>
    {actions.length > 0 && <div className="work-order-detail__actions">{actions.map((action) => <button key={action} data-testid={`transition-${action}`} className={action === '已取消' ? 'btn-danger' : action === '检修中' && order.status === '待验证' ? 'btn-secondary' : 'btn-primary'} onClick={() => setTarget(action)}>{action === '已接单' ? '确认接单' : action === '检修中' && order.status === '已接单' ? '开始检修' : action === '检修中' ? <><RotateCcw size={15} />退回检修</> : action === '待验证' ? '提交维修结果' : action === '已完成' ? '完成验证并闭环' : action === '已关闭' ? '关闭工单' : '取消工单'}</button>)}</div>}
    <div className="work-order-detail__grid mt-4">
      ...
      <aside className="work-order-detail__side">
        ...
        <IndustrialPanel title="工单信息"><div className="work-order-facts">{[['创建人', order.createdBy], ['来源', order.source === 'digital-twin' ? '3D数字孪生' : order.source === 'alert' ? '预警中心' : order.source ?? '现有流程'], ['数据模式', order.syncStatus ? '飞书协同' : 'Mock演示'], ['创建时间', new Date(order.createdTime).toLocaleString('zh-CN')], ['截止时间', new Date(order.deadline).toLocaleString('zh-CN')], ['最近同步', order.lastSyncedAt ? new Date(order.lastSyncedAt).toLocaleString('zh-CN') : '尚未同步'], ['负责人', order.assignee]].map(([label, value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div></IndustrialPanel>
    ...
    {['已完成', '已关闭'].includes(order.status) && <div className="mt-4"><AlertBanner tone="success" title={order.status === '已关闭' ? '工单已关闭' : '工单已完成'} description="关键结果已锁定，设备状态、库存、预警和维修历史已完成联动更新。" /></div>}
...
export function WorkOrdersPage() {
  ...
  return <div className="app-page work-orders-page"><PageHeader title="维修工单" description="以轨道式状态流程推进检修、验证和设备闭环" /><DemoDisclaimer compact />
    ...
    <DataTable className="mt-4">{rows.length ? <><thead><tr><th>工单编号 / 创建时间</th><th>设备</th><th>风险等级</th><th>故障描述</th><th>负责人</th><th>截止时间</th><th>同步 / 通知</th><th>状态</th><th>操作</th></tr></thead><tbody>{rows.map((order) => <tr key={order.workOrderId} className="cursor-pointer" onClick={() => navigate(`/work-orders/${order.workOrderId}`)}><td><div className="industrial-data work-order-id">{order.workOrderId}</div><div className="subtle mt-1">{new Date(order.createdTime).toLocaleString('zh-CN')}</div></td><td><strong className="text-[var(--industrial-text)]">{order.deviceName}</strong><div className="industrial-data subtle mt-1">{order.deviceId}</div></td><td><RiskBadge level={order.riskLevel} /></td><td><div className="max-w-xs truncate">{order.faultDescription}</div></td><td>{order.assignee}</td><td className={new Date(order.deadline) < new Date() && !['已完成', '已关闭', '已取消'].includes(order.status) ? 'text-[#ff858e]' : ''}>{new Date(order.deadline).toLocaleString('zh-CN')}</td><td><span className="work-order-sync"><i className={order.syncStatus === 'synced' ? 'is-synced' : ''} />{order.syncStatus === 'synced' ? '已同步' : order.syncStatus === 'failed' ? '同步失败' : '待同步'}</span><div className="subtle mt-1">卡片：{order.notificationStatus === 'sent' ? '已发送' : order.notificationStatus === 'failed' ? '失败' : '未发送'}</div></td><td><StatusBadge status={order.status} /></td><td><button className="btn-secondary min-h-8 px-2.5 py-1.5">查看处理</button></td></tr>)}</tbody></> : <tbody><tr><td><Empty /></td></tr></tbody>}</DataTable>
