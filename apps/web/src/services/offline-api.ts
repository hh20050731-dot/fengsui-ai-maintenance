...
export async function handleOfflineApi<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  else if (method === 'GET' && url.pathname === '/integration/status') result = { requestedMode: 'offline', effectiveMode: 'mock', degraded: true, partial: false, offlineDemo: true, feishuClient: false, capabilities: Object.fromEntries(['equipment', 'telemetry', 'health', 'alerts', 'workOrders', 'spareParts', 'spareTransactions', 'knowledge', 'operationLogs'].map((name) => [name, { mode: 'mock', configured: false }])), sso: '本地演示身份（离线）', bitable: '浏览器本地模拟数据', robot: '本地卡片预览', aiProvider: 'RuleBasedDiagnosisProvider（离线）', version: '1.0.2', lastSyncAt: nowIso(), missingConfig: [] };
