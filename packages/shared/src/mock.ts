...
const equipmentSeeds: EquipmentSeed[] = [
  ...
  { deviceId: 'FWP-001', deviceName: '1号给水泵', deviceType: '锅炉给水泵', systemArea: '给水系统', manufacturer: '上海凯士比', model: 'HGC4/10', ageDays: 1300, runningStatus: '运行', operatingCondition: '高负荷稳定运行', healthScore: 79, riskLevel: '关注', vibration: 4.8, temperature: 78, current: 103, pressure: 0.91, speed: 2980, runningHours: 20450, responsiblePerson: '赵工', responsibleUserId: 'zhao-gong', location: '汽机房给水泵区' },
  { deviceId: 'FWP-002', deviceName: '2号给水泵', deviceType: '锅炉给水泵', systemArea: '给水系统', manufacturer: '上海凯士比', model: 'HGC4/10', ageDays: 1290, runningStatus: '运行', operatingCondition: '稳定运行', healthScore: 93, riskLevel: '健康', vibration: 3.1, temperature: 65, current: 74, pressure: 0.65, speed: 2972, runningHours: 20190, responsiblePerson: '赵工', responsibleUserId: 'zhao-gong', location: '汽机房给水泵区' },
  ...
  { deviceId: 'PAF-001', deviceName: '一次风机', deviceType: '离心式风机', systemArea: '燃烧风系统', manufacturer: '金通灵', model: 'G4-73', ageDays: 1540, runningStatus: '运行', operatingCondition: '稳定运行', healthScore: 90, riskLevel: '健康', vibration: 3.3, temperature: 67, current: 71, pressure: 0.49, speed: 1450, runningHours: 24420, responsiblePerson: '陈工', responsibleUserId: 'chen-gong', location: '一次风机房' },
  { deviceId: 'SAF-001', deviceName: '二次风机', deviceType: '离心式风机', systemArea: '燃烧风系统', manufacturer: '金通灵', model: '9-26', ageDays: 1820, runningStatus: '运行', operatingCondition: '高负荷稳定运行', healthScore: 96, riskLevel: '健康', vibration: 2.2, temperature: 69, current: 86, pressure: 0.82, speed: 1480, runningHours: 30220, responsiblePerson: '刘工', responsibleUserId: 'liu-gong', location: '二次风机房' },
  { deviceId: 'CLP-001', deviceName: '冷却水泵', deviceType: '冷却水泵', systemArea: '闭式冷却水系统', manufacturer: '凯泉泵业', model: 'KQW200', ageDays: 1820, runningStatus: '运行', operatingCondition: '高负荷稳定运行', healthScore: 92, riskLevel: '健康', vibration: 2.8, temperature: 72, current: 96, pressure: 0.74, speed: 1475, runningHours: 30170, responsiblePerson: '周工', responsibleUserId: 'zhou-gong', location: '汽机房冷却水区' },
...
export function createMockEquipment(): Equipment[] {
  return equipmentSeeds.map(({ ageDays, ...item }) => ({
    ...
    trend24h: Array.from({ length: 8 }, (_, index) => Math.max(0, Math.min(100, Math.round(item.healthScore + Math.sin(index / 2) * 2)))),
    trend7d: Array.from({ length: 7 }, (_, index) => Math.max(0, Math.min(100, Math.round(item.healthScore + Math.sin(index / 1.7) * 3)))),
    trend30d: Array.from({ length: 10 }, (_, index) => Math.max(0, Math.min(100, Math.round(item.healthScore + Math.sin(index / 2.3) * 4)))),
    riskContributions: [
      { indicator: 'vibration', label: '振动', score: item.riskLevel === '健康' ? 2 : item.vibration, evidence: `当前振动 ${item.vibration} mm/s` },
      { indicator: 'temperature', label: '温度', score: item.riskLevel === '健康' ? 2 : Math.max(2, item.temperature - 65), evidence: `当前温度 ${item.temperature}℃` },
    ],
    recentAlertIds: item.riskLevel === '健康' ? [] : [`ALT-${item.deviceId}`],
    historicalWorkOrderIds: item.deviceId === 'IDF-001' ? ['WO-20260718-001'] : [],
    maintenanceRecordIds: [],
    recommendedMaintenanceAt: new Date(now().getTime() + (item.healthScore < 60 ? 4 : item.healthScore < 75 ? 24 : 72) * 3_600_000).toISOString(),
    modelType: item.deviceId.startsWith('IDF') ? 'induced-draft-fan' : item.deviceId.startsWith('FWP') ? 'feed-water-pump' : item.deviceId.startsWith('CWP') || item.deviceId.startsWith('CLP') ? 'circulation-water-pump' : item.deviceId.startsWith('GRB') ? 'grate-gearbox' : item.deviceId.startsWith('ACP') ? 'air-compressor' : item.deviceId.startsWith('LCP') ? 'leachate-pump' : 'industrial-motor',
  }));
}
...
export function createMockAlerts(): Alert[] {
  return [
    ...
    { alertId: 'ALT-20260716-003', deviceId: 'LCP-001', deviceName: '渗滤液处理泵', alertTime: ago(19), riskLevel: '高风险', healthScore: 55, operatingCondition: '停机', abnormalIndicators: ['出口压力', '机械密封'], indicatorContributions: { vibration: 4, temperature: 2, current: 3, pressure: 24, speed: 1 }, suspectedCause: '机械密封失效或泵体堵塞', aiAnalysis: '设备已进入检修停机状态，需完成隔离后检查泵体和机械密封。', maintenanceSuggestion: ['执行挂牌上锁', '拆检机械密封', '清理泵体'], suggestedDeadline: '立即安排', alertStatus: '处理中', confidence: 0.83, source: '模拟规则模型', relatedWorkOrderId: 'WO-20260716-003', acknowledgedBy: '马工', acknowledgedAt: ago(18.5) },
...
export function createMockWorkOrders(): WorkOrder[] {
  ...
  return [
    { id: 'WO-20260716-003', workOrderNo: 'WO-20260716-003', workOrderId: 'WO-20260716-003', sourceAlertId: 'ALT-20260716-003', deviceId: 'LCP-001', deviceName: '渗滤液处理泵', riskLevel: '高风险', faultPart: '出口与机械密封', faultType: '压力异常及密封渗漏', faultDescription: '出口压力异常并伴随机械密封渗漏', maintenanceSuggestion: ['执行安全隔离', '拆检机械密封和泵体'], assignee: '马工', assigneeUserId: 'ma-gong', createdBy: '黄浩', createdAt: createdTimes[0]!, createdTime: createdTimes[0]!, deadline: ago(-6), requiredSpareParts: [{ partId: 'SP-004', partName: '机械密封件', quantity: 1, unit: '套' }], consumedSpareParts: [], processingRecord: [{ id: 'REC-001', time: ago(18), operator: '马工', action: '开始检修', detail: '已完成停电、泄压和挂牌上锁' }], healthScoreBefore: 55, status: '检修中' },
